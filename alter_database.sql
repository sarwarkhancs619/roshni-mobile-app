...
ADD COLUMN IF NOT EXISTS assigned_house_id TEXT DEFAULT 'sunbal_house';

-- 2. Add house constraints check
ALTER TABLE public.friends DROP CONSTRAINT IF EXISTS friends_assigned_house_id_check;
ALTER TABLE public.friends ADD CONSTRAINT friends_assigned_house_id_check CHECK (assigned_house_id IN ('sunbal_house', 'amin_house', 'amina_house', 'roshni_house'));
...
ALTER TABLE public.profiles ADD CONSTRAINT profiles_workshop_id_check CHECK (workshop_id IN ('bakery', 'woodwork', 'farming', 'textile', 'artwork', 'sports', 'sunbal_house', 'amin_house', 'amina_house', 'roshni_house'));
